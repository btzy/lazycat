...
#else
    size_t approx_log2 = (sizeof(val) * 8) - __lzcnt(val | 1);
#endif
...
#if !defined(_MSC_VER)
    size_t approx_log2 = (sizeof(val) * 8 - 1) - __builtin_clzl(val | 1);
#else
    size_t approx_log2 = (sizeof(val) * 8 - 1) - __lzcnt(val | 1);
#endif
...
BENCHMARK_F(I32_Fixture, Write_I32_LazyCat_Pow8)(benchmark::State& state) {
    for (auto _ : state) {
        char arr[64];
        const auto writer = integral_writer_v4<std::int32_t>{{}, first, 0};
        if (writer.size() <= 64) {
            writer.write(arr);
        }
        benchmark::DoNotOptimize(arr);
